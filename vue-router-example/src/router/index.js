...
//实现路由懒加载的形式，只需要在引入文件的时候做个异步操作即可实现vue的懒加载功能
...
// const about = () => import(/* webpackChunkName: "group-about" */ '@/components/about')
...
let router = new Router({
  ...
  scrollBehavior(to,from,savePosition){//滚动行为
    ...
    if(savePosition){
      return savePosition //针对鼠标点击浏览器前进后退，如果记录到坐标存在就返回出来之前坐标，
    }else{
  ...
  routes: [
    {
      ...
      meta:{
        //路由元信息，可以通过路由守卫中的to对象来获取
        title:'Home-HelloWorld'
      }
    },
    {
      path: '/home',
      name: 'HelloWorld',//对路由进行命名，可以通过调用这个命名来调用这个路由
      component: HelloWorld,
      alias: '/abc',
